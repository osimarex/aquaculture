...
async function getHistoricFPI(numPoints: string = "All"): Promise<any> {
  ...
  try {
    ...
    let query = `
    SELECT ${pointsClause} *  
    FROM ${process.env.DB_FPI_TABLE} 
    ORDER BY Date ASC
  `;

    // console.log(query); // Log the query string to the console
    // console.log("Historic FPI data: ", query);
